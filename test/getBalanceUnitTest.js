...
contract('ROSCA getParticipantBalance Unit Test', function(accounts) {
    ...
    it("checks getParticipantBalance returns correct withdrawable value", co(function* () {
        ...
        yield utils.bid(2, consts.defaultPot() * 0.98);

        utils.increaseTime(consts.ROUND_PERIOD_IN_SECS);
        yield utils.startRound(rosca);
        let balance = yield utils.getParticipantBalance(2)

        let totalDiscounts = yield utils.totalDiscounts()
    ...
    it("checks that getParticipantBalance returns negative value for delinquents " +
       "(who haven't won the pot)", co(function* () {
        ...
        let balance = yield utils.getParticipantBalance(1);

        let expectedBalance = - consts.CONTRIBUTION_SIZE;
        assert.equal(balance, expectedBalance);
        utils.assertThrows(utils.withdraw(1));
        ...
        let debt = - balance;
        yield utils.contribute(1, debt + EXTRA_CONTRIBUTION)

        let contractBalanceBefore = web3.eth.getBalance(rosca.address);
        yield utils.withdraw(1);
        let contractBalanceAfter = web3.eth.getBalance(rosca.address);
    ...
    it("checks that getParticipantBalance returns negative value for delinquents " +
       "(who already won the pot)", co(function* () {
        ...
        yield Promise.all([
            ...
            utils.contribute(0, 5 * consts.CONTRIBUTION_SIZE, rosca),
            utils.contribute(1, 0.5 * consts.CONTRIBUTION_SIZE, rosca),
            utils.contribute(2, 0.5 * consts.CONTRIBUTION_SIZE, rosca),
        ]);
        ...
        let balance = yield utils.getParticipantBalance(winnerAddress, rosca);
