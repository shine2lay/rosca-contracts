...
contract('end of ROSCA unit test', function(accounts) {
    ...
    it("checks if endOfROSCARetrieve{Surplus,Fees} retrieve the funds when called in this order + check event",
      ...
      for (let rosca of [ethRosca, erc20Rosca]) {
        ...
        let result = yield rosca.endOfROSCARetrieveSurplus(0);
        ...
        contractCredit = yield rosca.contractNetCredit();
        assert.equal(contractCredit, 0);

        // Now retrieve fees
        let totalFees = yield rosca.totalFees();
        forepersonBalanceBefore = yield rosca.getBalance(0, tokenContract);
        yield rosca.endOfROSCARetrieveFees(0);
        forepersonBalanceAfter = yield rosca.getBalance(0, tokenContract);
    ...
    it("checks if endOfROSCARetrieve{Fees, Surplus} retrieve the funds when called in this order", co(function* () {
      ...
      let forepersonBalanceBefore = yield ethRosca.getBalance(0);
      yield ethRosca.endOfROSCARetrieveFees(0);
      let forepersonBalanceAfter = yield ethRosca.getBalance(0);
      ...
      forepersonBalanceBefore = yield ethRosca.getBalance(0);
      yield ethRosca.endOfROSCARetrieveSurplus(0);
      forepersonBalanceAfter = yield ethRosca.getBalance(0);

      utils.assertEqualUpToGasCosts(forepersonBalanceAfter - forepersonBalanceBefore, contractCredit);

      contractCredit = yield ethRosca.getBalance(ethRosca.address());
      assert.equal(contractCredit, 0);
    }));
    ...
    it("validates endOfROSCARetrieve{Surplus,Fee} throws if called not by the foreperson",
      ...
      yield utils.assertThrows(
        ethRosca.endOfROSCARetrieveSurplus(1));
      yield utils.assertThrows(
        ethRosca.endOfROSCARetrieveSurplus(9));

      yield utils.assertThrows(
        ethRosca.endOfROSCARetrieveFees(1));
      yield utils.assertThrows(
        ethRosca.endOfROSCARetrieveFees(9));
    }));
