...
contract('fees unit test', function(accounts) {
  ...
  it('does not charge overcontributions that do not get used in the ROSCA and do not get roscaHelper.withdrawn', co(function* () {
    const BID_TO_PLACE = 0.9 * consts.defaultPot()
    const INDIVIDUAL_DISCOUNT = (consts.defaultPot() - BID_TO_PLACE) / consts.memberCount()
    ...
    let withdrewAmount = yield roscaHelper.withdrawAndGetWithdrewAmount(0)
    // roscaHelper.withdrawal would be 0.5C(over roscaHelper.contributed) + (0.9 * 2C) * 0.99(fee) + (0.1 * 2 / 2)(totalDiscounts) * 0.99(fee)
  ...
  it('does not charge overcontributions that do not get used in the ROSCA and do get roscaHelper.withdrawn', co(function* () {
    const BID_TO_PLACE = 0.9 * consts.defaultPot()
    const INDIVIDUAL_DISCOUNT = (consts.defaultPot() - BID_TO_PLACE) / consts.memberCount()
    // In this test, accounts[0] roscaHelper.contributes 1.5C in round 1, then roscaHelper.withdraws, then roscaHelper.contributes another 1C in round 2 .
    utils.increaseTime(consts.START_TIME_DELAY + 200);
    yield Promise.all([
      roscaHelper.startRound(),
      roscaHelper.contribute(0, 1.5 * consts.CONTRIBUTION_SIZE),
