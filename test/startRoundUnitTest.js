...
contract('ROSCA startRound Unit Test', function(accounts) {
    ...
    it("Throws when calling startRound before roundStartTime (including round = 0)", co(function* () {
        for (let i = 0; i < consts.memberCount() + 1; i++) {
            yield utils.assertThrows(roscaHelper.startRound(), "expected calling startRound before roundStartTime to throw");
        ...
        assert.isOk(yield roscaHelper.getCurrentRosca().endOfROSCA.call());  // Unfortunately, we need to check the internal var directly.
    }));
});
